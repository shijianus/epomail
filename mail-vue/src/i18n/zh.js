const zh = {
    ...
    staticUiNotice: '未配置背景图片时不会应用静态UI，系统默认使用动态UI；只有在配置了背景图片时才会自动启用。',
    dynamicUiBadge: '内置 Canvas 粒子星空系统',
    dynamicUiFeatureDesc: '全站默认启用动态星空界面，支持 3D 视差感应、节点光晕与多态弹窗定制。未设置静态背景图时自动生效。',
    dynamicUiStatus: '运行中 (默认生效)',
    websiteTitleTooltip: '自定义全站网站标题，同时应用于动态与静态界面。',
    loginBoxOpacityTooltip: '调整传统静态界面的登录卡片透明度，使用动态界面时无效。',
    loginBackgroundTooltip: '设置传统静态界面的登录背景壁纸，使用动态界面时无效。未设置图片时系统保持动态UI，仅在有图片时启用静态UI。',
