<template>
  <div class="settings-container">
    ...
    <el-scrollbar class="scroll" v-if="!firstLoading">
        ...
        <div class="card-grid">
          ...
          <div class="settings-card">
            ...
            <div class="card-content">
              ...
              <div class="setting-item">
                <div class="title-item">
                  <span>{{ $t('globalEmailTitle') }}</span>
                  <el-tooltip effect="dark" :content="$t('globalEmailTooltip')">
                    <Icon class="warning" icon="fe:warning" width="18" height="18"/>
                  </el-tooltip>
                </div>
                <div class="forward">
                  <span>{{ $t('globalEmailAction') }}</span>
                  <el-button class="opt-button" size="small" type="primary" :title="$t('globalEmailSendTitle')" @click="openGlobalEmailDialog">
      ...
      <!-- Global Broadcast Email Fullscreen Dialog -->
      <el-dialog
        v-model="globalEmailDialogShow"
        :class="['welcome-dialog-canvas', 'global-email-dialog-canvas', { 'is-fullscreen': isGlobalEmailFullscreen }]"
        :close-on-click-modal="false"
        :show-close="false"
        @closed="closeGlobalEmailDialog"
        width="1140px"
        top="4vh"
      >
        <template #header>
          <div class="write-dialog-top">
            <!-- Left Group: Title -->
            <div class="top-left">
              <div class="quill-badge global-badge">
                <Icon icon="fluent:megaphone-loud-24-regular" width="20" height="20" />
              </div>
              <span class="dialog-main-title">{{ $t('globalEmailTitle') }}</span>
              <span class="official-channel-tag">{{ $t('globalEmailSenderOfficial') }}</span>
            </div>
            ...
            <!-- Right Group: Fullscreen Toggle & Close -->
            <div class="top-right">
              <el-tooltip :content="isGlobalEmailFullscreen ? $t('exitFullscreen') : $t('toggleFullscreen')" effect="dark" placement="bottom">
                <div class="tool-icon-btn top-action-btn" @click="toggleGlobalEmailFullscreen">
                  <Icon :icon="isGlobalEmailFullscreen ? 'fluent:full-screen-minimize-24-regular' : 'fluent:full-screen-maximize-24-regular'" width="20" height="20" />
                </div>
              </el-tooltip>
              <div class="tool-icon-btn close-icon-btn" :title="$t('close')" @click="globalEmailDialogShow = false">
                <Icon icon="material-symbols-light:close-rounded" width="22" height="22" />
              </div>
            </div>
          </div>
        ...
        <div class="welcome-dialog-body">
          ...
          <div class="welcome-recipients-row audience-selection-row">
            <div class="recipients-label">
              <Icon icon="solar:users-group-rounded-bold" width="16" height="16" class="recipients-icon" />
              <span>{{ $t('globalEmailAudience') }}:</span>
            </div>
            <div class="recipients-content audience-controls">
              <el-radio-group v-model="globalEmailForm.targetType" size="small">
                <el-radio-button value="all">{{ $t('globalEmailAudienceAll') }}</el-radio-button>
                <el-radio-button value="roles">{{ $t('globalEmailAudienceRoles') }}</el-radio-button>
              </el-radio-group>

              <div v-if="globalEmailForm.targetType === 'roles'" class="roles-selector-wrap">
                <el-select
                  v-model="globalEmailForm.targetRoleIds"
                  multiple
                  collapse-tags
                  collapse-tags-tooltip
                  :placeholder="$t('globalEmailSelectRoles')"
                  size="small"
                  style="width: 280px;"
                >
                  <el-option
          ...
          <div class="welcome-subject-bar">
            <el-input
              v-model="globalEmailForm.subject"
              size="large"
              :placeholder="$t('globalEmailSubjectPlaceholder')"
              class="write-subject-input"
            >
              <template #prefix>
                <Icon icon="fluent:text-bullet-list-square-sparkle-24-regular" width="18" height="18" class="subject-icon" />
          ...
          <!-- 4. Workspace Editor Card -->
          <div class="welcome-editor-card">
            <!-- Toolbar attached to top of editing area -->
            <div class="editor-toolbar-header">
              <!-- Left Group: Formatting / Helper Tools -->
              <div class="editor-left-tools">
              ...
              <div class="editor-right-tools">
                ...
                <el-tooltip :content="$t('clearContent')" effect="dark" placement="top">
                  <div class="tool-icon-btn danger-hover" @click="clearGlobalEmailContent">
                    <Icon icon="solar:trash-bin-trash-bold" width="17" height="17" />
                  </div>
                </el-tooltip>
                ...
                <el-tooltip :content="isGlobalEmailFullscreen ? $t('exitFullscreen') : $t('toggleFullscreen')" effect="dark" placement="top">
                  <div class="tool-icon-btn" @click="toggleGlobalEmailFullscreen">
                    <Icon :icon="isGlobalEmailFullscreen ? 'fluent:full-screen-minimize-24-regular' : 'fluent:full-screen-maximize-24-regular'" width="17" height="17" />
                  </div>
                </el-tooltip>
              </div>
            </div>
            ...
            <!-- Editor Mount Workspace -->
            <div class="editor-mount-area">
              <template v-if="globalEmailEditorFormat === 'rich'">
                <tinyEditor
                  editor-id="global-email-sys-editor"
                  :def-value="globalEmailForm.content"
                  ref="globalEditorRef"
                  @change="onGlobalContentChange"
                  class="dialog-tiny-editor"
                />
              </template>
              <template v-else>
        ...
        <template #footer>
          <!-- Bottom Action & Rules Bar -->
          <div class="welcome-fullscreen-footer">
            <div class="footer-left-rules">
              <!-- Retention TTL -->
              <div class="rule-item">
                <span class="rule-label">{{ $t('globalEmailExpireDays') }}:</span>
                <el-select v-model="globalEmailForm.expireDays" size="small" style="width: 170px;">
                  <el-option :value="7" :label="$t('welcomeExpire7Days')" />
                  <el-option :value="14" :label="$t('welcomeExpire14Days')" />
                  <el-option :value="30" :label="$t('welcomeExpire30Days')" />
                  <el-option :value="90" :label="$t('welcomeExpire90Days')" />
              ...
              <!-- Deliver to Future New Users Switch -->
              <div class="rule-item">
                <span class="rule-label">{{ $t('globalEmailSendToNew') }}:</span>
                <el-switch v-model="globalEmailForm.sendToNewUsers" :active-value="1" :inactive-value="0" size="small" />
                <el-tooltip effect="dark" :content="$t('globalEmailSendToNewDesc')" placement="top">
                  <Icon class="warning" icon="fe:warning" width="16" height="16" style="margin-left: 4px; cursor: pointer;"/>
                </el-tooltip>
              </div>

              <!-- Starred in Inbox Switch -->
              <div class="rule-item">
                <span class="rule-label">{{ $t('globalEmailIsStarred') }}:</span>
                <el-switch v-model="globalEmailForm.isStarred" :active-value="1" :inactive-value="0" size="small" />
                <el-tooltip effect="dark" :content="$t('globalEmailIsStarredDesc')" placement="top">
                  <Icon class="warning" icon="fe:warning" width="16" height="16" style="margin-left: 4px; cursor: pointer;"/>
                </el-tooltip>
              </div>
            </div>
            ...
            <div class="footer-right-actions">
              <el-tooltip :content="$t('globalEmailSaveDraft')" effect="dark" placement="top">
                <el-button
                  :loading="savingGlobalEmail"
                  @click="saveGlobalEmailDraft"
                  class="btn-save-secondary"
                >
                  <Icon icon="fluent:save-24-regular" width="16" height="16" style="margin-right: 6px;" />
                  {{ $t('globalEmailSaveDraft') }}
                </el-button>
              </el-tooltip>

              <el-tooltip :content="$t('globalEmailBroadcastBtn')" effect="dark" placement="top">
                <el-button
                  type="primary"
                  :loading="sendingGlobalEmail"
                  @click="confirmBroadcastGlobalEmail"
                  class="btn-broadcast-primary"
                >
                  <Icon icon="fluent:send-24-filled" width="17" height="17" style="margin-right: 6px;" />
                  {{ $t('globalEmailBroadcastBtn') }}
                </el-button>
              </el-tooltip>
            </div>
          </div>
...
const supportedWelcomeLangs = [
  { key: 'zh', label: '简体中文' },
  { key: 'zh-Hant', label: '正體中文' },
...
const adminDefaultLangKey = computed(() => {
  const l = (settingStore.lang || 'zh').toLowerCase()
...
function getWelcomeLangName(key) {
  const target = supportedWelcomeLangs.find(item => item.key === key)
  return target ? target.label : key
}

const activeWelcomeLang = ref('zh')
const welcomeTemplatesMap = reactive({
  zh: { subject: '', content: '' },
...
// Global Announcement Email State
const globalEmailDialogShow = ref(false)
const isGlobalEmailFullscreen = ref(false)
const globalEmailEditorFormat = ref('rich')
const globalEditorRef = ref(null)
const sendingGlobalEmail = ref(false)
const savingGlobalEmail = ref(false)
const availableRoles = ref([])

const globalEmailForm = reactive({
  subject: '',
  content: '',
  text: '',
  targetType: 'all', // 'all' | 'roles'
  targetRoleIds: [],
  expireDays: 30,
  sendToNewUsers: 1,
  isStarred: 1
...
function openWelcomeEmailSetting() {
  ...
  let savedTemplates = {}
  try {
    if (typeof setting.value.welcomeTemplates === 'string') {
      savedTemplates = JSON.parse(setting.value.welcomeTemplates)
    } else if (typeof setting.value.welcomeTemplates === 'object' && setting.value.welcomeTemplates) {
      savedTemplates = setting.value.welcomeTemplates
    }
  } catch (e) {}

  // 3. Populate welcomeTemplatesMap for all supported languages
  supportedWelcomeLangs.forEach(item => {
    const key = item.key
    const defTpl = getWelcomeTemplate(key)
    const saved = savedTemplates[key] || {}
    let subj = saved.subject
    let cnt = saved.content
    if (key === 'zh' && !subj && setting.value.welcomeSubject) {
      subj = setting.value.welcomeSubject
    }
    if (key === 'zh' && !cnt && setting.value.welcomeContent) {
      cnt = setting.value.welcomeContent
    }
    welcomeTemplatesMap[key] = {
      subject: subj || defTpl.subject,
      content: cnt || defTpl.content
    }
  })

  // 4. Load active template into form
  const currentTpl = welcomeTemplatesMap[activeWelcomeLang.value] || getWelcomeTemplate(activeWelcomeLang.value)
  welcomeEmailForm.welcomeSubject = currentTpl.subject
  welcomeEmailForm.welcomeContent = currentTpl.content
...
function resetToDefaultWelcomeTemplate() {
  const defTpl = getWelcomeTemplate(activeWelcomeLang.value)
  welcomeTemplatesMap[activeWelcomeLang.value] = {
    subject: defTpl.subject,
    content: defTpl.content
  }
  welcomeEmailForm.welcomeSubject = defTpl.subject
  welcomeEmailForm.welcomeContent = defTpl.content
  if (welcomeEditorFormat.value === 'rich' && welcomeEditorRef.value) {
    welcomeEditorRef.value.clearEditor()
    nextTick(() => {
      if (welcomeEditorRef.value) {
        welcomeEditorRef.value.setContent(defTpl.content)
      }
    })
  }
...
function setEditorFormat(format) {
  ...
  if (format === 'rich') {
    ...
    if (!welcomeEmailForm.welcomeContent || !welcomeEmailForm.welcomeContent.trim()) {
      welcomeEmailForm.welcomeContent = (welcomeTemplatesMap[activeWelcomeLang.value] || getWelcomeTemplate(activeWelcomeLang.value)).content
    }
...
function saveWelcomeTemplate() {
  ...
  const payload = {
    ...
    welcomeTemplates: JSON.stringify(welcomeTemplatesMap),
    welcomeLang: activeWelcomeLang.value,
...
function confirmBroadcastWelcome() {
  ...
  const langLabel = getWelcomeLangName(activeWelcomeLang.value)
  const langNotice = t('welcomeEmailBroadcastLangNotice', { lang: langLabel }) || `当前将以【${langLabel}】版本向全员发送欢迎邮件`
  const baseConfirm = t('broadcastConfirmHighRiskMsg') || t('welcomeBroadcastConfirmMsg')
...
function toggleGlobalEmailFullscreen() {
  isGlobalEmailFullscreen.value = !isGlobalEmailFullscreen.value
}

function setGlobalEmailEditorFormat(format) {
  if (globalEmailEditorFormat.value === format) return
  if (format === 'rich') {
    let content = globalEmailForm.content || ''
    content = compileMarkdownToHtml(content)
    globalEmailForm.content = content
    globalEmailEditorFormat.value = 'rich'
    nextTick(() => {
      if (globalEditorRef.value && globalEditorRef.value.setContent) {
        globalEditorRef.value.setContent(content)
      }
    })
  } else {
    if (globalEditorRef.value && globalEditorRef.value.getContent) {
      try {
        const current = globalEditorRef.value.getContent()
        if (current && current.trim()) {
          globalEmailForm.content = current
        }
      } catch (e) {}
    }
    globalEmailEditorFormat.value = 'source'
  }
}

function clearGlobalEmailContent() {
  globalEmailForm.content = ''
  if (globalEmailEditorFormat.value === 'rich' && globalEditorRef.value) {
    globalEditorRef.value.clearEditor()
  }
}

function onGlobalContentChange(content) {
  globalEmailForm.content = content
}
...
function insertGlobalSyntax(prefix, suffix = '', defaultText = '') {
  ...
  const start = textarea.selectionStart || 0
  const end = textarea.selectionEnd || 0
  const text = globalEmailForm.content || ''
  const selected = text.substring(start, end)
  const innerText = selected || defaultText
  const replacement = `${prefix}${innerText}${suffix}`
  globalEmailForm.content = text.substring(0, start) + replacement + text.substring(end)
  nextTick(() => {
    textarea.focus()
    const selectStart = start + prefix.length
    const selectEnd = selectStart + innerText.length
...
function saveGlobalEmailDraft() {
  if (savingGlobalEmail.value) return
  let finalContent = globalEmailForm.content || ''
  if (globalEmailEditorFormat.value === 'rich' && globalEditorRef.value) {
    const current = globalEditorRef.value.getContent()
    if (current !== undefined) finalContent = current
  }
  finalContent = compileMarkdownToHtml(finalContent)
  globalEmailForm.content = finalContent

  savingGlobalEmail.value = true
  const payload = {
    subject: globalEmailForm.subject,
    content: finalContent,
    ...
    sendToNewUsers: Number(globalEmailForm.sendToNewUsers),
    isStarred: Number(globalEmailForm.isStarred)
  }
...
function confirmBroadcastGlobalEmail() {
  if (sendingGlobalEmail.value) return
  let finalContent = globalEmailForm.content || ''
  if (globalEmailEditorFormat.value === 'rich' && globalEditorRef.value) {
    const current = globalEditorRef.value.getContent()
    if (current !== undefined) finalContent = current
