      ...
      <div v-else-if="!authInfo.isLoggedIn" class="login-prompt-state">
        ...
        <div class="brand-connection-bar">
          <div class="brand-chip epomail-chip" title="Epomail Identity">
            <img src="/logo.svg" alt="Epomail" class="epomail-logo-img" />
          </div>
          ...
          <!-- App Chip (Direct Tab Icon Display) -->
          <div 
            class="brand-chip app-chip" 
            :class="{ 'is-shijianus': isShijianusBlog }" 
            :style="(!resolvedAppLogo || logoLoadFailed) ? { background: getAvatarBg(authInfo.app?.name) } : {}" 
            :title="authInfo.app?.name"
          >
            <img 
              v-if="resolvedAppLogo && !logoLoadFailed" 
              :src="resolvedAppLogo" 
              :alt="authInfo.app?.name" 
              class="app-chip-img"
...
// 扫描并获取应用的现成标签页展示图片 (Favicon / Tab Icon)
const resolvedAppLogo = computed(() => {
  if (logoLoadFailed.value) {
    if (isShijianusBlog.value) {
      return '/shijianus-favicon.png'
    }
    return ''
  }

  // 1. 如果已有明确配置且合规的 logoUrl
  if (authInfo.app?.logoUrl && !authInfo.app.logoUrl.endsWith('.svg')) {
    return authInfo.app.logoUrl
  }

  // 2. 如果是 shijianus-blog，优先直接使用标签页现成展示图片 (favicon.png)
  if (isShijianusBlog.value) {
    return 'https://blog.epocanvas.com/favicon.png'
  }

  if (authInfo.app?.logoUrl) {
    return authInfo.app.logoUrl
  }

  // 3. 扫描第三方应用主站的现成标签页图标
  if (authInfo.app?.homepageUrl) {
    try {
      const u = new URL(authInfo.app.homepageUrl)
...
function handleLogoError() {
  if (isShijianusBlog.value && resolvedAppLogo.value !== '/shijianus-favicon.png') {
    authInfo.app.logoUrl = '/shijianus-favicon.png'
    return
  }
